## P0面试优先级（2026）
- `P0-1` 缓存三大问题：穿透/击穿/雪崩及组合治理方案。
- `P0-2` 持久化与恢复：RDB/AOF/AOF重写、数据丢失窗口。
- `P0-3` 高可用：主从复制、哨兵、Cluster 分片与扩缩容。
- `P0-4` 过期与淘汰：过期删除策略、`maxmemory` 淘汰策略。
- `P0-5` 业务实战：分布式锁、热点 key、大 key 治理。

## P0常见误区修正（本页已修订）
- Redis 不是“绝对单线程”：命令执行单线程为主，网络 I/O 在高版本可多线程。
- 分布式锁不能只用 `setnx`，应使用 `SET key val NX EX` + Lua 释放。
- 缓存一致性不是“删缓存即可”，还要设计重试/补偿与兜底。
...
1. **纯内存访问**。数据存放在内存中，内存的响应时间大约是100纳秒，这是Redis高吞吐的重要基础（单机通常十万级 QPS）。
2. **多路复用I/O**，Redis采用事件驱动 + I/O多路复用（Linux常见epoll）来处理大量连接，减少阻塞等待和线程切换开销。
...
5. **网络与协议优化**
	* RESP协议：简单文本协议（支持二进制数据），解析高效，降低序列化开销。
	- Pipeline支持：批量发送多个命令，合并网络往返延迟，提升吞吐量（如一次性写入100条命令）。
	- 长连接复用：减少TCP握手/挥手开销。
6. **分布式扩展能力**
	* Cluster分片：数据分散到16384个Slot，多节点并行处理请求，突破单机瓶颈。
	- 主从复制：读写分离，从节点分担读负载，复制过程异步进行。

> 6.x 以后引入 I/O 线程用于网络读写；命令执行仍主要在主线程中
...
**Redis实际采用的删除策略只有两种**：**惰性删除（Lazy Expiration） + 定期删除（Periodic Expiration）**。

### 定期删除（Periodic Expiration）

**Redis 周期性随机抽取部分 Key（默认每秒 10 次，通过 `hz` 配置），删除其中已过期的 Key。**
    
执行流程：
    1. 随机选取 20 个 Key（数量可调）。
    2. 删除其中已过期的 Key。
    3. 若过期 Key 比例超过 25%，重复步骤 1。
...
**当客户端访问一个 Key 时，Redis 会检查其 TTL，若已过期则立即删除。**
...
**缺点**：可能导致大量过期 Key 未被访问而长期占用内存（内存泄露风险）。

> **生产建议**：惰性删除 + 定期删除组合使用，平衡内存和 CPU 效率。
> 
> “Redis没有使用定时删除策略，因为当前时间事件（Time Event）的实现是无序链表，查找复杂度O(N)，不适合处理大量精细定时任务。”
...
> **子进程直接扫描内存中的当前数据**，根据数据类型生成等效的写入命令
> * 例如：某个 Hash 键当前内存中有字段 `{f1: v1, f2: v2}`，直接生成 `HMSET hash_key f1 v1 f2 v2`。
> * 而非解析历史命令（如 `HSET hash_key f1 v1` + `HSET hash_key f2 v2`）。*
...
> * 避免重写阻塞
> 	* 重写时会读取Redis中的key，需要确保内存充足
> * 大 Key 处理
> 	* 拆分超过 10MB 的 Hash/ZSet，防止重写时子进程阻塞过久。
> 	* 若某个 Hash 有 100 万字段，子进程需遍历生成巨大 `HMSET` 命令，可能导致：
> 		* 内存峰值（COW 复制大 Key 修改时的内存页）
> 		* 磁盘 I/O 暴增（写入超大 AOF 临时文件）
> * 磁盘空间预留
> 	* 预留 2 倍当前 AOF 文件大小的磁盘空间
### 混合持久化
...
- `multi`：用于标记事务块的开始,Redis会将后续的命令逐个放入队列中，然后使用`exec`原子化地执行这个命令队列
- `exec`：执行命令队列
- `discard`：清除命令队列
- `watch`：监视key
- `unwatch`：清除监视key
...
Redis集群采用的算法是**哈希槽分区算法**。Redis集群中有16384个哈希槽（槽的范围是 0 -16383，哈希槽），将不同的哈希槽分布在不同的Redis节点上面进行管理，也就是说每个Redis节点只负责一部分的哈希槽。在对数据进行操作的时候，集群会对使用CRC16算法对key进行计算并对16384取模（slot = **CRC16(key)%16384**），得到的结果就是 Key-Value 所放入的槽，通过这个值，去找到对应的槽所对应的Redis节点，然后直接到这个对应的节点上进行存取操作。
...
1. clusterNode数据结构：保存节点的当前状态，比如节点的创建时间，节点的名字，节点当前的配置纪元，节点的IP和地址等等。
...
### 缓存三大问题对比
- 穿透：查不存在数据 -> 布隆过滤 + 空值缓存
- 击穿：热点 key 过期瞬时打 DB -> 互斥重建 + 预热
- 雪崩：大面积同时失效 -> 过期时间打散 + 限流降级
...
| **类型**   | **现象**                 | **影响**           |
| -------- | ---------------------- | ---------------- |
| **访问倾斜** | 少数Key（如热点商品）被高频访问      | 单节点CPU/带宽过载，延迟飙升 |
| **数据倾斜** | 大Key集中到某节点（如10GB的Hash） | 内存溢出，迁移阻塞        |
| **分片倾斜** | 分片算法不均导致节点负载差异>50%     | 集群整体吞吐下降         |
### 核心成因分析

1. **业务场景导致的热点Key**

- 高频访问：热搜榜单、秒杀商品、明星动态。
- 大Key聚合：全量用户配置、未分页的排行榜数据。

 2. **分片策略缺陷**

- **哈希算法不均**：原生`CRC16`分片在特定Key模式（如连续ID）下可能分布不均。
- **Hash Tag滥用**：过度使用`{}`强制Key同分片（如`{product}:iphone15`），人为制造热点。

3. **数据冷热分离不足**

- 冷数据未淘汰：历史数据长期占用内存，挤压热点数据空间。
- 热数据无备份：单分片扛所有读请求。
### 系统性解决方案
...
1. **热点Key治理（访问倾斜）**
	...
	| **策略**      | **实现方式**                                                            | **适用场景**     |
	| ----------- | ------------------------------------------------------------------- | ------------ |
	| **本地缓存**    | 客户端缓存热点Key（Guava/Caffeine），降低Redis请求压力                              | 读多写少，容忍短暂不一致 |
	| **多级缓存**    | Redis + Memcached 分层，分散热点                                           | 超高并发场景（如秒杀）  |
	| **Key分片拆分** | 将`product:iphone15`拆为`product:iphone15:v1`、`product:iphone15:v2`... | 大Key访问倾斜     |
	| **读写分离**    | 热点Key配置从节点，分担读流量                                                    | 读密集型业务       |
	...
	public String getProductInfo(String id) {
	    // 1. 先查本地缓存
	    String value = localCache.get(id);
	    if (value != null) return value; 
	    
	    // 2. 查Redis（含热点标记）
	    value = redis.get(id);
	    if ("HOT".equals(redis.tag(id))) {
	        localCache.put(id, value, 5); // 短期缓存
	    }
	    return value;
	}
	```
...
2. **数据分片优化（存储倾斜）**
	* **动态分片算法**  
		替换原生CRC16，采用一致性哈希或范围分片（需代理层支持，如Twemproxy）。
	* **大Key拆分**
		```bash
	    # 原始大Key → 分片小Key
	    HMSET config:global ...   # 10GB
	    # 拆分为：
	    HMSET config:shard1 ...
	    HMSET config:shard2 ...
	    ```
...
3. **集群架构升级（分片倾斜）**
	...
	- **增加副本数**  
	    为热点分片配置多个从节点（如1主3从），通过负载均衡引流。
	- **槽位手动调整**  
	    迁移高负载节点槽位到低负载节点：
    
		```bash
	    
	    redis-cli --cluster reshard --cluster-from <高负载节点ID> \
	              --cluster-to <低负载节点ID> --cluster-slots 500
	    ```
	- **使用Proxy层**  
	    通过Codis/Redis Cluster Proxy实现请求二次调度，绕过热点节点。
    
###  一致性保障与流量控制

1. **热点探测与动态路由**
	* **实时监控**：通过`redis-cli --hotkeys`或自定义脚本识别热点Key。
	* **动态分片**：
		```python
	    # 伪代码：根据QPS动态路由
	    def route_request(key):
	        if redis.get_qps(key) > 10000:   # 超过阈值
	            return random.choice([":6379", ":6380"])  # 分流到多个节点
	        else:
	            return default_node(key)
	    ```
	
2. **熔断与降级**
	- 配置Sentinel规则，当节点QPS超过阈值时触发熔断，请求降级至DB或默认值。
	- 示例（Sentinel规则）：
		```bash
	    sentinel monitor hot-node 192.168.1.1 6379 2
	    sentinel down-after-milliseconds hot-node 500 # 500ms超时判定失效
	    ```
...
|**场景**|**解决方案**|**关键指标**|
|---|---|---|
|突发流量热点|本地缓存 + 多级缓存|本地缓存命中率 >90%|
|持续大Key存储|分片拆分 + 压缩存储|单Key大小 <1MB|
|集群分片不均|手动槽位迁移 + Proxy调度|节点负载差异 <20%|
|长期访问倾斜|读写分离 + 动态扩容|从节点读流量占比 >70%|

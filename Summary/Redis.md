...
Redis的String能表达3种值的类型：字符串、整数、浮点数。100.01 是个六位的串字符串对象的编码可以是int、raw或者embstr。
...
[Redisson](Redis-extend#Redisson)

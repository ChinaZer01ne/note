...
2. **多路复用I/O**，Redis采用select(默认会根据操作系统进行选择O(1)的调用方式，比如epoll/kqueue/evport，如果没有则选择select)作为I/O多路复用技术的实现，再加上Redis自身的事件处理模型将epoll中的链接，读写关闭都转换为了时间，不在I/O上浪费过多的时间。
...
[Redis数据结构](Redis-extends.md#Redis的数据结构)
[Redis底层数据结构](Redis-extends.md#底层数据结构)
...
[RDB文件结构](Redis-extends.md#RDB文件结构)
...
[Redis集群的搭建](Redis-extends.md#Redis集群的搭建)

## Redis集群的运维：

[Redis集群的运维](Redis-extends.md#Redis集群的运维)
...
[Lua脚本](Redis-extends.md#Lua脚本)

# 发布与订阅

[Redis发布订阅](Redis-extends.md#发布与订阅)
...
[慢查询日志](Redis-extends.md#Redis慢日志查询)
...
[Redisson](Redis-extends.md#Redisson)

## P0面试优先级（2026）
- `P0-1` 架构原则：高内聚低耦合、边界清晰、可演进。
- `P0-2` 非功能设计：性能、可用性、扩展性、安全性。
- `P0-3` 一致性与可用性：CAP 取舍与业务映射。
- `P0-4` 可观测与运维：日志、指标、追踪、告警闭环。
- `P0-5` 成本意识：资源成本、复杂度成本、组织成本。

## P0常见误区修正（本页已修订）
- 架构设计不是“画图比赛”，要可落地可验证。
- 过度抽象会抬高维护成本，先满足当前核心诉求。
- 高可用要按故障场景设计，不是堆组件。
...
1. 高性能
	> 高性能意味着系统拥有更大流量的处理能力，更低的响应延迟。例如 1 秒可处理 10W 并发请求，接口响应时间 5 ms 等等。
2. 高可用
...
Write Behind Caching Pattern（写后缓存模式）是一种缓存策略，它在**写入数据时将更新操作首先应用于缓存，然后异步地将数据写入持久存储**。与传统的同步写入策略相比，写后缓存模式延迟了数据写入持久存储的时机，以提高写入操作的性能和吞吐量。
1. 写入数据：
...
> 从一个单位时间到另一个单位时间，限流会重置，可能会有两倍的流量过来，导致无法精确控制流量
...
这个架构模型非常简单，客户端请求进来，业务应用读写数据库，返回结果，非常好理解。但需要注意的是，这里的数据库是「单机」部署的，所以它有一个致命的缺点：
...
数据库越大，意味故障恢复时间越久。那按照前面我们提到的「高可用」标准，这个方案可能连 1 个 9 都达不到，远远无法满足我们对可用性的要求。
...
这样确实会大大降低出问题的概率。但我们依旧不能掉以轻心，因为无论怎么分散，它们总归还是在一个相同的环境下：**机房**。到底该怎么应对机房级别的故障呢？
...
同城灾备的最大优势在于，我们再也不用担心「机房」级别的故障了，一个机房发生风险，我们只需把流量切换到另一个机房即可，可用性再次提高。
...
因为两个机房都能处理业务请求，这对我们系统的内部维护、改造、升级提供了更多的可实施空间（流量随时切换），现在，整个系统的弹性也变大了。那这种架构有什么问题呢？
...
* 加权轮询
	* 简单的实现是生成权重比例重复节点，然后轮询列表，保证每个节点都能访问
	* Dubbo的实现挺有意思
* 加权随机轮询：随机按权重来
* 最少活跃数
	* 优先选择连接数少的服务

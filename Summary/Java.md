...
>  * 通过位运算提高效率
#### HashMap的put流程（JDK 8）::
1. 对 key 做 hash 扰动，计算桶下标 `index = (n - 1) & hash`。
2. 桶为空：直接放入新节点。
3. 桶非空：
   - key 相同：覆盖旧值。
   - key 不同：链表尾插；达到树化条件时转红黑树。
4. 新增成功后 `size++`，若超过阈值触发 `resize()`。

#### 为什么树化阈值是 8，退化阈值是 6？::
- 树化是空间换时间，阈值太低会增加维护成本。
- 8 是工程折中点：低碰撞时仍保留链表简单性，高碰撞时保障最坏复杂度。
- 退化阈值设为 6 是为了避免在 7/8 附近频繁树链切换（滞回）。
#### HashMap有什么并发问题？::  
* 1.7：头插
...
#### CHM高频追问::
- `get()` 为什么无锁：依赖 `volatile` 可见性 + 不可逆结构发布，读路径不加互斥锁。
- `size()` 是否绝对准确：并发场景下是近似快照，监控够用，强一致统计需业务兜底。
- 为什么不允许 `null` key/value：避免 `get()==null` 无法区分“没有key”还是“value为null”的并发歧义。
- JDK8 为何用 `synchronized`：锁粒度已缩到桶，且 JVM 对内置锁优化成熟，配合 CAS 更简洁。
...
> 关于`lastRun`：
> - `transfer()` 迁移链表时会先找“最后一段 run（lastRun）”，这段节点在新表中的目标桶相同。
> - 对这段 run 直接复用原链表，前半段再按高位 bit 拆分重建。
> - 作用：减少新节点创建与指针改写，降低扩容期间的对象分配和复制开销。
...
| 问题类型           | 优先方案                          | 说明                   |
| -------------- | ----------------------------- | -------------------- |
| 复合读改写（如 `i++`） | `synchronized` / `Lock` / 原子类 | 只用 `volatile` 不保证原子性 |
| 状态可见性（停止标志）    | `volatile`                    | 轻量、低开销               |
| 高并发计数          | `LongAdder`                   | 写多读少优于 `AtomicLong`  |
| Map 并发读写       | `ConcurrentHashMap`           | 避免 `HashMap` 并发风险    |
## Java的内存模型
### 什么是JMM？::

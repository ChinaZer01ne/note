...
![](https://secure2.wostatic.cn/static/6sGp9TRtx3hdVynbN2QZTR/image.png)
...
![](https://secure2.wostatic.cn/static/pujQXTdbW1gVav6zkrmzHy/image.png)
...
spring.factories配置⽂件中增加了`RibbonAutoConfiguration` 的自动配置
...
![](https://secure2.wostatic.cn/static/tRGkXm4bTuWiqH29wNhgYz/image.png)
...
public class AutodeliverController {
    ...
    public Integer findResumeOpenStateTimeout(@PathVariable Long userId) {
        String url = "http://lagou-serviceresume/resume/openstate/" + userId; 
...
/**
  ...
  * 1 登记注册
  * 2 分发优惠券（并不是核⼼步骤），
  * 这⾥如果调⽤优惠券微服务返回了异常信息或者是熔断后的错误信息，这些信息如果抛给⽤户很不友好,此时，我们可以返回⼀个兜底数据，预设的默认值（服务降级）
  **
  */
}
```

![](https://secure2.wostatic.cn/static/58QWvJkvhSv5Yqyu2kS84P/image.png)
...
![](https://secure2.wostatic.cn/static/tHSg39Z8DT86Kiy2n9Srz2/image.png)

正常返回

![](https://secure2.wostatic.cn/static/dBmrNV7A9iEHs8jni374R1/image.png)

## Hystrix舱壁模式（线程池隔离策略）

![](https://secure2.wostatic.cn/static/etfNdVEZZurUCrEAgkadNC/image.png)
...
![](https://secure2.wostatic.cn/static/jfpzNTRkQ22j7FWofwoRk2/image.png)
...
我们上述通过注解进⾏的配置也可以配置在配置⽂件中，全局公共的
...
访问健康检查接⼝： [http://localhost:8090/actuator/health](http://localhost:8090/actuator/health)

hystrix正常⼯作状态

![](https://secure2.wostatic.cn/static/mXhHuCNF8uAyW4wSBAAaSc/image.png)

跳闸状态

![](https://secure2.wostatic.cn/static/7AY5jPcL262eaSbiZq8xt1/image.png)

活动窗⼝内⾃我修复

![](https://secure2.wostatic.cn/static/6b7SHGuXGgE9j8SJ56NwFy/image.png)
...
![](https://secure2.wostatic.cn/static/n4sv1VyPpaBjSupb2siA2H/image.png)

![](https://secure2.wostatic.cn/static/xboaH9cZhDz76gZP29JhZA/image.png)
...
4. 在被监测的微服务中注册监控servlet，后期我们就是通过访问这个servlet来获取该服务的Hystrix监控数据的，还有一个前提是被监控的微服务需要引入springboot的actuator功能
...
![](https://secure2.wostatic.cn/static/jt7buiw7up3cBb5mDgFDPq/image.png)
```

5. 访问测试http://localhost:9000/hystrix
    
    ![](https://secure2.wostatic.cn/static/aFm5TnEpcu7b7KH9yczJ8d/image.png)
    
    输⼊监控的微服务端点地址，展示监控的详细数据，⽐如监控服务消费者[http://localhost:8090/actuator/hystrix.stream](http://localhost:8090/actuator/hystrix.stream)
    
    ![](https://secure2.wostatic.cn/static/w7QuPLApmnMfEkG117cpqW/image.png)
...
![](https://secure2.wostatic.cn/static/vWZVub1JQjL6k52qizHB47/image.png)

![](https://secure2.wostatic.cn/static/a5ejNUpxjnpG2VxXV3aAMy/image.png)
...
4. 浏览器访问Turbine项⽬， [http://localhost:9001/turbine.stream，就可以看到](http://localhost:9001/turbine.stream%EF%BC%8C%E5%B0%B1%E5%8F%AF%E4%BB%A5%E7%9C%8B%E5%88%B0)监控数据了
    
    ![](https://secure2.wostatic.cn/static/2UvNipxeDfEtmDUV1Kf6Ms/image.png)
    
    我们通过dashboard的⻚⾯查看数据更直观，把刚才的地址输⼊dashboard地址栏
    
    ![](https://secure2.wostatic.cn/static/rGs6ATvk8WP5Zb44dmjnhM/image.png)
    
    监控⻚⾯
    
    ![](https://secure2.wostatic.cn/static/9pharmkFVH3JvbLEu1zS8t/image.png)
...
```
这个注解中导入了`EnableCircuitBreakerImportSelector`类
```

2. 查看EnableCircuitBreakerImportSelector类
...
```
重点看一下此类继承了`SpringFactoryImportSelector`类，并且泛型传入的是`EnableCircuitBreaker`
```
...
public abstract class SpringFactoryImportSelector<T>
  ...
  // 选择导入的类，具体逻辑是根据传入的泛型类的全名作为key去spring.factories文件中查找对应的类，然后注入
  @Override
...
```
在spring-cloud-netflix-hystrix包中spring.factories⽂件内容中有配置
```
...
3. 配置类`org.springframework.cloud.netflix.hystrix.HystrixCircuitBreakerConfiguration`

```java
@Configuration(proxyBeanMethods = false)
public class HystrixCircuitBreakerConfiguration {

  // 向容器中注入了切面类，这是Hystrix的重要的类
  @Bean
...
4. 切面`com.netflix.hystrix.contrib.javanica.aop.aspectj.HystrixCommandAspect`
...
public Object methodsAnnotatedWithHystrixCommand(final ProceedingJoinPoint joinPoint) throws Throwable {
    ...
    // 获取HystrixInvokable 对象
    HystrixInvokable invokable = HystrixCommandFactory.getInstance().create(metaHolder);
    ExecutionType executionType = metaHolder.isCollapserAnnotationPresent() ?
            metaHolder.getCollapserExecutionType() : metaHolder.getExecutionType();

    // 执行方法：同步、异步、observable
    Object result;
    try {
        // 非observable类型，使用CommandExecutor执行
...
protected AbstractCommand(HystrixCommandGroupKey group, HystrixCommandKey key, HystrixThreadPoolKey threadPoolKey, HystrixCircuitBreaker circuitBreaker, HystrixThreadPool threadPool,
    ...
    // 线程池初始化，内部使用ConcurrentHashMap来缓存
...
static HystrixThreadPool getInstance(HystrixThreadPoolKey threadPoolKey, HystrixThreadPoolProperties.Setter propertiesBuilder) {
    ...
    // threadPools是ConcurrentHashMap结构
...
![](https://secure2.wostatic.cn/static/gsf4cUtZaFsebcf4vcZxK2/image.png)

接下来回到环绕通知⽅法，进⼊execute执⾏这⾥
...
public Future<R> queue() {
     ...
     */
     // Future的获取，业务的逻辑的执行，异常后对回退方法的调用一些列的处理都是用RxJava响应式编程的内容
...
![](https://secure2.wostatic.cn/static/5cbDV4wazm1s8qWkTjaxuX/image.png)
...
```
Feign的超时时⻓设置那其实就上⾯Ribbon的超时时⻓设置Hystrix超时设置（就按照之前Hystrix设置的⽅式就OK了）  

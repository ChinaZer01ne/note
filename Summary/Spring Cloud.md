## P0面试优先级（2026）
- `P0-1` 服务治理全链路：注册发现、网关、配置中心。
- `P0-2` 调用可靠性：超时、重试、熔断、限流与降级。
- `P0-3` 配置与发布：灰度发布、动态配置、回滚策略。
- `P0-4` 观测体系：调用链追踪、指标告警、故障定位。
- `P0-5` 迁移策略：Eureka/Hystrix 旧栈到新治理组件迁移。

## P0常见误区修正（本页已修订）
- Spring Cloud 不是单组件能力，而是治理体系协同。
- 重试配置不当会放大故障，需要限流和熔断配合。
- 老栈迁移要关注版本兼容与行为差异。

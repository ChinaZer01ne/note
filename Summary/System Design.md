...
> 1. **澄清需求与范围：** 主动提问，明确系统要做什么（功能）和做到什么程度（非功能）。
> 2. **估算与规模：** 进行粗略的容量估算（用户量、QPS、存储量），为设计决策提供依据。
> 3. **高层架构：** 勾勒出核心组件及其交互，说明技术选型理由。
> 4. **深入设计核心模块：** 聚焦最关键、最复杂的部分（如短链生成、重定向）。
> 5. **解决难点与权衡：** 讨论设计中遇到的挑战（如高并发读、ID生成）以及你如何权衡不同方案。
> 6. **考虑扩展性、容错等：** 思考系统如何应对增长和故障。
> 7. **收尾与总结：** 简要回顾关键设计点，或提出可能的优化方向。

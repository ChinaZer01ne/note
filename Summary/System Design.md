...
1. **动态存储（发件箱）**：
    ...
    - **选型**：**MySQL分库分表 + Redis缓存**。
        ...
        - 分片策略：按`用户ID Hash`（确保同一用户动态存储在相同分片）38。
...
2. **粉丝关系存储**：
    ...
    - **选型**：**图数据库（Neo4j）或 MySQL**。
        ...
        - 高频查询：获取用户的粉丝列表（推模式用）和关注列表（拉模式用）37。
...
3. **异步消费者**：从Kafka读取消息 → 将动态ID插入每个粉丝的Redis ZSET（`ZADD Timeline:{粉丝ID} {时间戳} {动态ID}`）59。
...
- **优化1：缓存大V最新动态**  
    使用Redis缓存大V最近100条动态，避免频繁查库39。
    
- **优化2：并行拉取**  
    对关注的大V并发请求，减少总耗时（线程池配置）9。
...
1. **主动对比方案**：
    ...
    > “推模式写扩散适合普通用户，但大V会导致写风暴；拉模式读扩散适合大V，但普通用户会引发多次查询。因此混合模式是平衡点。” 59
...
3. **关注工程细节**：
    ...
    > “Kafka分区分区键用发布者ID，保证同一用户的动态顺序写入相同分区，避免乱序。” 5
...
通过这个案例，你已掌握**需求分析→架构选型→细节设计→问题攻防**的完整链条。下次面试遇到类似问题（如朋友圈、微博），只需调整关注关系（双向/单向）和排序策略（时间/算法），即可灵活应对！

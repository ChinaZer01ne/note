## P0面试优先级（2026）
- `P0-1` 需求澄清：功能范围、SLA、容量目标、成本约束。
- `P0-2` 容量估算：QPS/并发连接/带宽/存储分开估算。
- `P0-3` 核心链路：读写路径、缓存层、异步化、降级兜底。
- `P0-4` 稳定性：限流、熔断、隔离、超时与重试策略。
- `P0-5` 数据一致性：强一致、最终一致、补偿事务方案。

## P0常见误区修正（本页已修订）
- 并发连接数不等于 QPS，二者需按请求频次换算。
- 高并发不只靠“加机器”，需要缓存、削峰和治理策略。
- 方案优劣取决于业务约束，不存在万能架构。

## 如何进行系统设计？::
### 高并发白板讲解模板
1. 业务目标与 SLA
2. 容量估算（并发连接/QPS/带宽/存储）
3. 架构路径（缓存/MQ/分库分表/降级）
4. 指标结果与权衡
...
* 估算与规模
	...
	* 用户量，假设10亿
	* 10亿 * 10% = 1亿 DAU日活
	* 1亿 * 10% = 1000万同时在线（并发连接）；QPS 需结合人均请求频次换算
...
>* `链接过期时间`：短连接是否需要设置有效期？（这里要确认！）
>* `访问统计`：是否需要记录短连接的点击次数、时间、来源等？（这里要确认！）
...
1. **全局唯一性**：这是最根本的要求，绝对不能出现重复的订单号。
2. **趋势递增**：通常使用递增的序列，这样有利于数据库索引（如InnoDB的B+Tree），提高写入和查询效率。
3. **安全性**：订单号不应透露公司的敏感信息，如“一天有多少订单”。避免使用连续自增（如1,2,3...），以防被猜测和爬取。
4. **可读性（可选但推荐）**：包含一定的信息，如日期，便于人工识别和排查问题。
5. **防止并发冲突**：在高并发场景下，生成订单号的方法必须保证不会出现重复。
6. **控制长度**：不宜过长，通常在16-20位之间，便于沟通和传输。

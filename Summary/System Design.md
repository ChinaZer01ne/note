...
- **核心功能：**
	...
	- `缩短(longUrl) -> shortUrl`：用户输入一个长URL，系统生成并返回一个对应的短URL（如 `https://short.com/abc123`)。
		
	- `重定向(shortUrl) -> longUrl`：用户访问短URL，系统将其重定向（HTTP 301/302）到原始的长URL。
		
	- （可选）`自定义短码`：用户能否指定自定义短码（如 `https://short.com/mybrand`)？_（这里要确认！）_
		
	- （可选）`链接过期时间`：短链接是否需要设置有效期？_（这里要确认！）_
		
	- （可选）`访问统计`：是否需要记录短链接的点击次数、时间、来源等？_（这里要确认！）_
...
- **面试官互动：** 面试官可能会回答你的问题，例如：“核心功能就是生成和重定向，不需要自定义和统计。可用性99.9%，重定向延迟P99<100ms。数据量按你估算的规模来，需要持久化不能丢。安全方面考虑基本的防刷就行。”
...
- “基于这些需求，我构想的高层架构主要包含以下组件：
    ...
    - **架构图示意：**
        ...
        用户 -> [API Gateway (Spring Boot)] 
                   |
                   | (POST /shorten)   -> [短链生成服务] -> [MySQL] (写映射)  -> [Redis] (Set映射)
                   | (GET /:shortCode) -> [重定向服务]  -> [Redis] (Get映射) -> [MySQL] (缓存未命中时读) 
                   |                                     |
                   | <------------------------------------(返回301/302重定向响应)
        

**第三步：深入设计核心模块 - 短码生成与存储**
...
1. **主动沟通：** 第一步澄清需求**至关重要**！不要假设。多问“是否”、“多少”、“优先级”。
    
2. **估算意识：** 随时进行数量级估算（QPS、存储），让设计有依据。面试官常看这个。
    
3. **分步演进：** 从简单开始（单机单DB），逐步引入复杂性（缓存、集群、分片）以解决问题。解释为什么需要下一步。
    
4. **权衡取舍：** 这是**灵魂**！每个决策（如选MySQL vs NoSQL，强一致 vs 最终一致，Cache-Aside vs Write-Through）都要说出优缺点和选择理由（基于之前确认的需求！）。常用句式：“方案A优点是...缺点是...；方案B优点是...缺点是...；因为我们的需求是...，所以我选择...”。
    
5. **聚焦核心：** 时间有限，优先深入讲解最核心、最挑战的部分（如短码生成、缓存策略、高并发读设计）。
    
6. **承认未知：** 如果对某个技术细节不确定（如Redis Cluster内部协议），诚实说“我对这个细节了解不够深入，但在我的设计中，会采用Redis Cluster来解决分片和高可用问题，具体配置我会查阅最佳实践”。然后回到你熟悉的部分。
    
7. **可视化：** 善用白板或画图工具画出组件、数据流。图形化表达更清晰。
    
8. **保持积极讨论：** 面试是双向讨论。如果面试官提出不同意见或挑战，把它看作展示你解决问题能力的机会，冷静分析回应。
    

通过这个短链系统的例子，你应该能体会到系统设计面试的答题脉络和思考方式。接下来，找其他经典题目（如设计Twitter Feed流、设计抢购系统）反复练习这个框架，重点关注需求澄清、估算、核心机制设计和权衡取舍。加油！💪
## Feed

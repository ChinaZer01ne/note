## P0面试优先级（2026）
- `P0-1` 注册发现模型：Namespace/Group/Service/Cluster。
- `P0-2` 健康检查与实例剔除：临时实例与永久实例差异。
- `P0-3` 配置中心：DataId 规范、灰度发布、动态刷新。
- `P0-4` 可用性治理：集群部署、持久化、容灾与降级。
- `P0-5` 版本兼容：Boot/Cloud/SCA 版本矩阵与升级策略。

## P0常见误区修正（本页已修订）
- Nacos 不是“配完就稳”，集群与数据库都要纳入高可用设计。
- 服务发现可用不等于业务可用，仍需超时重试和熔断兜底。
- 版本不匹配是常见线上问题，需按官方矩阵选型。
...
- 下载解压安装包，执行命令启动(版本请按官方版本矩阵选择（下方仅为历史示例）)
...
## 注册中心参数基线
- 心跳、实例剔除、保护阈值
- Namespace/Group 隔离策略
- 配置发布灰度与回滚

## 服务提供者注册到Nacos

- 在父pom中引入SCA依赖（版本号仅示例，实际请按官方版本矩阵）
...
Nacos客户端引入的时候，会关联引入Ribbon的依赖包，我们使用OpenFeign的时候也会引入Ribbon的依赖，Ribbon包括Hystrix都按原来方式进行配置即可。此处，我们将建立微服务，又启动了一个8083端口，注册到Nacos上，便于测试负载均衡，我们通过后台也可以看出。

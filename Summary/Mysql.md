## P0面试优先级（2026）
- `P0-1` 索引与执行计划：B+Tree、最左前缀、覆盖索引、回表、`EXPLAIN` 关键列。
- `P0-2` 事务与隔离级别：MVCC、ReadView、幻读与间隙锁。
- `P0-3` 锁机制：行锁/间隙锁/临键锁、死锁定位与规避。
- `P0-4` 日志体系：Undo/Redo/Binlog、两阶段提交与崩溃恢复。
- `P0-5` 高可用与扩展：主从复制、延迟治理、分库分表边界。

## P0常见误区修正（本页已修订）
- 索引并非“建了就快”，低区分度列和错误查询写法会让索引失效。
- RR 级别不是“完全无并发问题”，仍需关注锁冲突与死锁。
- `EXPLAIN` 只看 `type=ref` 不够，还要结合 `rows`、`filtered`、`extra`。
...
* SQL解析（解析器）
	* 词法分析、语法分析
* 预处理器
	* 参数校验、权限校验；Prepared Statement 可复用执行计划（由客户端/服务端协作）
* 优化器（查询优化）
	* 成本分析，找到成本最小的执行计划
* 执行器（生成执行计划）

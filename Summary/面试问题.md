...
5. 一个线程如果对变量只有读操作，没有写操作，由于JMM的原因，那这个变量不会直接刷到主内存
6. JMM
7. 解决缓存不一致的问题
	1. 总线锁
	2. 缓存一致性协议
8. 原子性：8中原子性操作，注意a = b不是原子性，虽然是复制但是是两条指令read b; assign a;
9. 可见性：volatile， 锁
10. 有序性：happens-before原则
11. A a = new A(); 这行代码中的引用、对象、class对象都在内存中的什么位置?
	1. a在栈中
	2. A对象在堆中
	3. A.class对象在堆中，这个对象作为访问方法区数据的外部接口
	4. A的类元数据在方法区中
12. future设计
13. 单线程的指令重排序和多线程的指令重排序
...
18. unsafe
	1. 底层的CPU指令
		...
		5. lock1：volatile
		6. membar-acquire
19. exchanger：只适合2个线程
20. StampedLock
21. complationService

22. complatableFuture
23. `synchronized` 和 `Lock` 的区别？如何选择？
...
25. 公平锁和非公平锁有什么区别？`ReentrantLock` 默认是什么？为什么默认是非公平的？
26. 什么是读写锁？适用于什么场景？`ReentrantReadWriteLock` 的规则是什么？

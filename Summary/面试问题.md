...
#### 1. 讲一讲Redis常见数据结构以及使用场景  
#### 1. Redis的Key，Value有大小限制吗？
#### 1. Redis存储hash的话用什么命令？
#### 1. 如果想把set存到集群里怎么做？
#### 1. Redis什么时候底层数据结构会发生变化？
#### 1. keys命令和scan命令有什么区别？
#### 1. pipeline有什么好处，为什么要用 pipeline？  
#### 1. Redis 持久化方式有哪些？以及有什么区别？  
#### 1. 什么是Redis事务？原理是什么？  
#### 1. 如何在100个亿URL中快速判断某URL是否存在？  
#### 1. 统计高并发网站每个网页每天的 UV 数据，结合Redis你会如何实现？  
#### 1. 说一说Redis的Key和Value的数据结构组织?  
#### 1. 渐进式rehash听过没？讲一讲！  
#### 1. 讲一讲Redis分布式锁的实现  
#### 1. Redlock听过没？讲一讲！  
#### 1. 什么是缓存击穿？该如何解决  
...
#### 1. 什么是BigKey？该如何解决  
#### 1. 什么是热点Key？该如何解决  
#### 1. Redis某个热点key在某一个分片上，这种数据倾斜的问题怎么解决的？
#### 1. redis过期策略都有哪些？LRU 算法知道吗？  
	1. 和普通的LRU不一样  
#### 1. 讲一讲Redis缓存的数据一致性问题和处理方案
#### 1. 在项目中缓存是如何使用的？缓存如果使用不当会造成什么后果？
#### 1. 如何保证Redis高并发、高可用、持久化？Redis的主从复制原理能介绍一下么？Redis的哨兵原理能介绍一下么？
#### 1. Redis的持久化有哪几种方式？不同的持久化机制都有什么优缺点？持久化机制具体底层是如何实现的？
#### 1. Redis集群模式的工作原理能说一下么？在集群模式下，Redis的key是如何寻址的？分布式寻址都有哪些算法？了解一致性hash算法吗？如何动态增加和删除一个节点？
#### 1. Redis的并发竞争问题是什么？如何解决这个问题？了解Redis事务的CAS方案吗？
#### 1. 生产环境中的redis是怎么部署的？
#### 1. Redis分布式锁怎么做的？
#### 1. Redision红锁原理
#### 1. 超卖问题怎么解决？串行？串行的话你的性能不就浪费了吗？
#### 1. Redis的lua的表达式有研究过吗？讲讲
...
#### 1. 为什么要进行系统拆分？如何进行系统拆分？拆分后如何通信？
#### 1. 分布式服务接口的幂等性如何设计（比如不能重复扣款）？
#### 1. 分布式服务接口请求的顺序性如何保证？
#### 1. 如何自己设计一个rpc框架？
#### 1. 使用Redis如何设计分布式锁？使用Zookeeper来设计分布式锁可以吗？这两种分布式锁的实现方式哪种效率比较高？
...
1. 
#### 1. Guava如何做分布式缓存的
#### 1. 16. 讲一下你对SpringCloud的一些组件的了解？
#### 1. 这些相关的组件哪些组件停止维护了？
#### 1. 他们停止维护了，你们有没有想过换一下？
#### 1. 工作中你有遇到过什么高并发的场景么？
#### 1. 你们压力测试有吗？

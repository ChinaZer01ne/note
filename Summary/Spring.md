#flashcards 
## P0面试优先级（2026）
- `P0-1` IoC容器启动主链路：`refresh()` -> `BeanFactoryPostProcessor` -> `BeanPostProcessor` -> 单例Bean初始化。
- `P0-2` Bean生命周期与扩展点：`BeanFactoryPostProcessor`、`BeanPostProcessor`、`Aware`、`InitializingBean`。
- `P0-3` 循环依赖与三级缓存：仅单例 + 属性注入场景可解，重点讲清三级缓存职责。
- `P0-4` AOP与事务：代理创建时机、事务失效场景、传播行为、隔离级别、回滚规则。
- `P0-5` Spring Boot自动装配：`@EnableAutoConfiguration`、`AutoConfigurationImportSelector`、`AutoConfiguration.imports`。
- `P0-6` MVC主流程与常见扩展：`DispatcherServlet`、`HandlerMapping`、`HandlerAdapter`、`ViewResolver`。

## P0常见误区修正（本页已修订）
- Spring Bean 默认作用域是 `singleton`，不是“都不是单例”。
- 三级缓存中放的是 `ObjectFactory`（通常返回 early reference），不是 `FactoryBean`。
- 事务AOP不是“只基于CGLIB”，可能是 JDK 动态代理或 CGLIB。
- 传播行为正确枚举是 `REQUIRES_NEW`、`NOT_SUPPORTED`。
- Spring Boot 3 自动装配主要读取 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`。
# Spring初始化流程
...
其根本原因是：Bean 还在创建过程中（尚未完成初始化）就再次被依赖，导致“创建中再次创建”的递归依赖。**创建的对象但没完成初始化的对象不会提前暴露出来**
...
核心原因不是“设计如此”，而是为了**同时满足提前暴露与代理一致性**：
- 一级缓存 `singletonObjects`：存放完整可用的单例 Bean。
- 二级缓存 `earlySingletonObjects`：存放提前暴露的早期引用（可能是代理）。
- 三级缓存 `singletonFactories`：存放 `ObjectFactory`，按需生成 early reference（避免所有 Bean 都提前创建代理）。

如果只有二级缓存，通常难以兼顾“延迟创建代理”和“同一个 Bean 最终引用一致”这两个目标。
...
* 三级缓存如何解决？
	* 通过三级缓存存放 `ObjectFactory`，按需暴露 early reference（可能是代理）
...
我们通过去掉一级缓存的方式，将原先从二级缓存转移到一级缓存的地方去掉，把完成品的对象放在二级缓存中，是否可以？
完全可以。相当于用二级缓存代替了一级缓存，不影响Bean创建的流程。
但不建议。一级缓存和二级缓存职责不同：一级缓存表示“完整可用 Bean”，二级缓存表示“早期引用”。混用会让生命周期语义和缓存语义变混乱。
...
| REQUIRES_NEW | **(常用)** 挂起当前事务，开启一个全新事务。                            |
| SUPPORTS     | 如果有事务在运行，当前的方法就在这个事务内运行，否则他可以不运行在事务中                    |
| NOT_SUPPORTED | 当前的方法不应该运行在事务中，如果有运行的事务，将其挂起                            |
	...
	* @SpringBootApplication -> 
		* @EnableAutoConfiguration -> 
			* @Import(**AutoConfigurationImportSelector.class**)
				...
				* Spring Boot 2.x：读取 `spring.factories` 中 `EnableAutoConfiguration` 配置
# 如何优化SpringBoot的启动速度？

* 设置bean的懒加载
* 创建扫描索引`@Index`以及`spring-context-indexer`
* 减少`@ComponentScan`、`@SpringBootApplication`扫描类的范围，以及其他组件的扫描范围，比如swagger、feign等等。
* 关闭SpringBoot的JMX监控，`spring.jmx.enabled=false`
* 谨慎使用关闭字节码校验参数（现代JDK中不建议作为常规优化手段）
* 控制AOP切点范围，避免过宽切点导致启动阶段扫描和代理开销过大
* 关闭endpoint的一些监控功能
* 排除项目多余的依赖jar

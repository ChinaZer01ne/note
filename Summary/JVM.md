#flashcards 
## P0面试优先级（2026）
- `P0-1` JVM运行时数据区：线程私有/线程共享、对象分配路径、TLAB。
- `P0-2` 类加载机制：加载-连接-初始化、双亲委派、打破双亲委派场景（SPI/容器）。
- `P0-3` 垃圾回收主线：可达性分析、分代思想、G1关键流程与调优指标。
- `P0-4` 故障排查：Full GC 频繁、OOM（堆/元空间/直接内存）定位路径与常用命令。
- `P0-5` 性能调优：吞吐/延迟目标、参数取舍、压测验证与回归。

## P0常见误区修正（本页已修订）
- 现代JDK（9+）已无“扩展类加载器 + ext目录”模型，改为 Platform ClassLoader。
- 双亲委派主流程在 `ClassLoader#loadClass`，`findClass` 是子类扩展点。
- 字节码校验参数不应作为常规优化手段，需结合JDK版本与风险评估。
# JVM包括哪些部分？::
- 类加载子系统  
- 运行时数据区  
...
![](/Summary/image/JVM/JVM结构图.png)

> 线程私有的：Java栈、本地方法栈、程序计数器，堆中Eden区的TLAB
# 类加载机制
...
**符号引用验证的目的是确保解析动作能正常执行。验证阶段对安全性和正确性很重要。现代JDK下不建议把关闭字节码校验(`-Xverify:none`参数)作为常规优化手段，只有在明确版本前提和风险可控时才考虑。**
...
为类的静态变量赋予正确的初始值，主要是执行`<cinit>`方法，先执行父类，再执行子类。接口中也存在`<cinit>`方法，虚拟机保证`<cinit>`方法线程安全。
...
* 被系统类加载器和平台类加载器（JDK9+）加载的类型在运行期间不太可能被卸载，因为这些加载器实例通常长期可达，其 unreachable 的可能性极小。
* 自定义类加载器只有在简单的上下文环境中才能被卸载，而且需借助强制调用虚拟机的垃圾收集功能才可以做到。

> 类很难被卸载，不存在子类，Class对象没有被引用并且类加载器被回收，该类才能被卸载

## 类加载器有哪些？::

### JDK 8 及以前
- 启动类加载器（Bootstrap ClassLoader）
  - JVM内置实现（HotSpot中通常是C/C++实现），加载核心类库。
- 扩展类加载器（Extension ClassLoader）
  - 负责加载扩展目录下类库（历史机制）。
- 应用程序类加载器（Application ClassLoader）
  - 也称系统类加载器，加载应用Classpath下的类和资源。

### JDK 9+
- 启动类加载器（Bootstrap ClassLoader）
- 平台类加载器（Platform ClassLoader）
  - 取代旧的 Extension ClassLoader。
- 应用程序类加载器（Application ClassLoader）

### 自定义类加载器
- 继承 `ClassLoader` 并重写 `findClass`，按需从网络、数据库、加密文件等来源加载字节码。

> 备注：`rt.jar`、`sun.boot.class.path`、`java.ext.dirs` 等属于旧版本JDK语义，面试回答请标注版本。
## 一个类只会被加载一次吗？::
### 类加载器的命名空间
...
> 命名空间由该类加载器及所有父加载器所加载的类组成
## 什么是双亲委派机制? ::
类加载器按照从上到下的顺序形成了一个层次结构，即双亲委派模型。该模型可以确保类的隔离性和安全性，并减少类的重复加载。

当一个类加载器接收到类加载请求时，它会按照以下步骤进行，具体实现主流程可查看`ClassLoader#loadClass`方法（`findClass`是子类扩展点）：
...
触发Minor GC的时候会根据老年代空间担保等规则分配对象。可能会将新生代对象晋升到老年代，然后分配新对象；也可能没有足够的空间完成晋升，导致Full GC，Full GC后还是没有足够的空间，就会触发OOM。具体参考[老年代空间担保](#什么是老年代空间分配担保？)
...
Metasapce由多个​​虚拟空间​​​组成，虚拟空间就是系统中的一些连续的内存空间，它们按需分配。分配时，虚拟空间从系统中保留内存，但是还没有提交，Metaspace中的​​reserved​​就是所有的虚拟空间总和。

虚拟空间中的分配单元是一个一个的​`​meta chunk`​​​，当一个新的`chunk`从虚拟空间中被分配时，它所相关联的内存也被提交，Metaspace中的`​​committed`​​就是所有的`chunk`总和

新的chunk的内存分配 
* 在空闲列表中寻找已存在的空闲`chunk` 
* 如果没有找到合适的`chunk`，那么会在当前的虚拟空间中重新分配一个 
* 如果当前的虚拟空间中已经没有内存可用了，那么会​​`reserve:预定`​​一个新的虚拟空间 

类元数据被分配在一个个的`chunk`中，一个`chunk`可能不会包含多个`classloader`，但是一个`classloader`可能会横跨多个`chunk`，Metaspace中的​`​used`​​就是所有的包含了类元数据信息的`chunk` 

Metaspace中的`​​capacity`​​​就是已经分配内存的`chunk`总和，包括没有被​​`used`​​的。
...
* 如果是单核，并且没有停顿时间的要求，使用串行Serial，或者让JVM自己选
* 如果允许停顿时间大于1秒，选择并行垃圾收集器(ParNew, Parallel)或JVM自己选
* 如果响应时间重要，并且不能超过1秒，使用并发垃圾收集器，CMS/G1
...
垃圾收集的过程，是对象图由白到黑推进的过程。

### 并发标记（三色标记）阶段存在的问题？::
由于垃圾回收线程和用户线程一起执行，由于引用会发生变化，那么就会出现两种情况
* 对象消失：不能回收的对象被回收了，白色对象又被引用到了黑色对象上。
* 浮动垃圾：需要回收的没有被回收，这种情况问题不大，虽然有浮动垃圾，但可以放到下次GC

当且仅当同时满足下面两个条件的时候会发生“对象消失”的情况：
* 赋值器插入了一条或多条从黑色对象到白色对象的新引用。
* 赋值器删除了全部从灰色对象到该白色对象的直接或间接引用。
> 为什么是满足这两个条件？当增加了黑色对象到白色对象的引用不就会产生对象消失的问题吗？原因是，如果一个白色对象没有被任何对象引用，那么他就不会被增加一条从黑色对象来的引用，因为这个对象根本访问不到，所以他一定会首先有一条灰色对象的引用。
...
年龄达到一定阈值的对象会被移动到年老代中，没有达到的对象会被复制到"To"区域。Minor GC会一直重复这样的过程，直到"To"区被填满，"To"区被填满之后，会将所有对象移动到年老代中。

#### 2. 动态对象年龄判断

当前放对象的Survivor区域里，一批相同年龄的对象的总大小大于了这块Survivor区域的内存大小的50%，那么此时大于等于这批对象年龄的对象，就可以直接进入老年代了。

假设这个图里的Survivor2区有两个对象，这俩对象的年龄一样，都是2岁，然后俩对象加起来对象超过了50MB，超过了Survivor2区的内存大小的一半了，这个时候，Survivor2区里的大于等于2岁的对象，就要全部进入老年代里去。这就是所谓的动态年龄判断的规则，这条规则也会让一些新生代的对象进入老年代。

![](对象进入老年代.webp)

逻辑：年龄1+年龄2+年龄n的多个年龄对象总和超过了Survivor区域的50%，此时就会把年龄n以上的对象都放入老年代。

可通过参数`TargetSurvivorRatio`参数配置比率。

说白了，无论是15岁的那个规则，还是动态年龄判断的规则，都是希望那些可能是长期存活的对象，尽早进入老年代，避免频繁复制。

#### 3. 大对象直接进入老年代

多大叫大对象？有一个JVM参数，就是"`-XX:PretenureSizeThreshold`"，可以把他的值设置为字节数，比如"1048576"字节，就是1MB。

他的意思就是，如果你要创建一个大于这个大小的对象，比如一个超大的数组，或者是别的啥东西，此时就直接把这个大对象放到老年代里去。压根儿不会经过新生代。

之所以这么做，就是要避免新生代里出现那种大对象，然后屡次躲过GC，还得把他在两个Survivor区域里来回复制多次之后才能进入老年代，那么大的一个对象在内存里来回复制，会很耗费时间。比如缓存的大对像。

#### 4. Minor GC后的对象太多无法放入Survivor区

假设在发生GC的时候，发现Eden区里超过150MB的存活对象（参考上图Survivor区容量100M），此时没办法放入Survivor区中，此时该怎么办呢？这个时候就必须得把这些对象直接转移到老年代去。

### 什么是老年代空间分配担保？

首先，在执行任何一次Minor GC之前，JVM会先检查一下老年代可用的内存空间，是否大于新生代所有对象的总大小。为啥检查这个呢？因为最极端的情况下，可能新生代Minor GC过后，所有对象都存活下来了，或者，Minor GC后的对象太多无法放入Survivor区，那岂不是新生代所有对象全部要进入老年代？如图：

![](空间分配担保.webp)

如果说发现老年代的内存大小是大于新生代所有对象的，此时就可以放心大胆的对新生代发起一次Minor GC了，因为即使Minor GC之后所有对象都存活，Survivor区放不下了，也可以转移到老年代去。那么这个时候是不是有可能在Minor GC之后新生代的对象全部存活下来，然后全部需要转移到老年代去。

但是老年代空间又不够了呢？发现老年代的可用内存已经小于了新生代的全部对象大小了，如：新生代有800M垃圾，但老年代只有300M可用空间了，这时就会看一个"`-XX:-HandlePromotionFailure`"的参数是否设置了。

这个参数，就是看看老年代的内存大小，是否大于之前每一次Minor GC后进入老年代的对象的平均大小，JDK后面的版本默认设置了。

举个例子，之前每次Minor GC后，平均都有10MB左右的对象会进入老年代，那么此时老年代可用内存大于10MB。这就说明，很可能这次Minor GC过后也是差不多10MB左右的对象会进入老年代，此时老年代空间是够的，如下图：

![](HandlePromotionFailure.webp)

如果上面那个步骤判断失败了，或者是"`-XX:-HandlePromotionFailure`"参数没设置，此时就会直接触发一次老年代垃圾回收（Full GC），尽量腾出来一些内存空间，然后再执行Minor GC。如果上面两个步骤都判断成功了，那么就是说可以冒点风险尝试一下Minor GC。
![](老年代空间分配担保.png)

**此时进行Minor GC后会有几种处理可能：**

1. Minor GC过后，剩余的存活对象的大小，是小于Survivor区的大小的，那么此时存活对象进入Survivor区域。
2. Minor GC过后，剩余的存活对象的大小，是大于 Survivor区域的大小，但是是小于老年代可用内存大小的，此时就直接进入老年代。
3. 很不幸，Minor GC过后，剩余的存活对象的大小，大于了Survivor区域的大小，也大于了老年代可用内存的大小。此时老年代都放不下这些存活对象了，就会发生"`Handle Promotion Failure`"的情况，这个时候就会触发一次"Full GC"，同时也会对新生代进行垃圾回收。
...
1、调用`System.gc()`方法，会建议JVM执行Full GC，但不一定会执行，通常会执行，所以平时不要调用该方法。

2、在Minor GC的时候，老年代可用内存小于新生代全部对象的大小，如果没开启空间担保参数，会直接触发Full GC，所以一般空间担保参数都会打开。

3、老年代可用内存小于历次新生代GC后进入老年代的平均对象大小，此时会提前Full GC；但是"`-XX:HandlePromotionFailure`"参数，在JDK 1.6以后就被废弃了，所以现在一般都不会在生产环境里设置这个参数了。在JDK 1.6以后，只要判断"老年代可用空间"大于"新生代对象总和"或者"老年代可用空间"大于"历次Minor GC升入老年代对象的平均大小"，两个条件满足一个，就可以直接进行Minor GC，不需要提前触发Full GC了。

4、**是新生代Minor GC后的存活对象大于Survivor，那么就会进入老年代，此时老年代内存不足，触发Full GC**。这里的不足就是判断条件后还是不足或者经过判断后进行YGC后放入老年代此时的空间不足，然后进行Full GC，就会出现频繁Full GC。达到一定情况后，就会OOM了。

5、是如果用的是CMS收集器，老年代可用内存大于历次新生代GC后进入老年代的对象平均大小，但是老年代已经使用的内存空间超过了（"`-XX:CMSInitiatingOccupancyFaction=92%`"JDK6默认值 ）这个参数指定的比例，也会自动触发Full GC。

6、**大对象导致Full GC**，由于大对象可能直接进入老年代，又因为CMS不会进行空间压缩，当老年代由于内存碎片，没有足够的连续空间的时候，放不下新生代存活的对象，就会导致Full GC。

7、**方法区满了**，会进行Full GC。

> 线上Full GC频繁的原因，大概率就是YGC回收不掉，导致存活太多对象；或者是CMS中老年代对象大小超过阈值，机器内存配置太低；还有就是内存碎片。
> 
> 简单点讲：老年代空间不足就会Full GC
...
* 初始标记（Initial Marking） 
	* 仅仅是**标记一下GC Roots能直接关联到的对象**，并且修改TAMS指针的值，让下一阶段用户线程并发运行时，能正确的在可用的Region中分配新对象。这个过程需要STW，但耗时很短，而且是借用进行Minor GC的时候同步完成的，所以G1收集器在这个阶段实际并没有额外的停顿。
* 并发标记（Concurrent Marking）
	* 从GC Roots开始对堆中对象进行**可达性分析**，递归扫描整个堆里的对象图，找出要回收的对象，这阶段耗时较长，但可与用户线程并发执行，当对象图扫描完成后，还要重新处理SATB记录下的并发过程中引用变动的对象。
	* 通过“[**原始快照**](#并发标记阶段存在的问题？)”的方式（**SATB**）解决对象消失的问题。
	* 通过[**TAMS**](#并发标记阶段存在的问题？)来解决新分配对象被回收的问题。
* 最终标记（Final Marking）
	* 对用户线程做另一个短暂的暂停，用于**处理并发阶段结束后仍遗留下来的少量的SATB记录**
	* STW
...
> 由于一个Region可能引用别的Region的对象，通过Remembered Set避免全局扫描。引用写操作的时候会记录Remembered Set
...
#### 如何排查？
- 排查SOP：先确认是否异常（频率、停顿时长、业务影响），再采集证据（GC日志、heap直方图、线程栈），最后输出结论（参数、代码、容量三类改进）。
- 输出物：Full GC 排查流程图（现象 -> 取证 -> 定位 -> 处置 -> 复盘）、GC 日志速记卡（触发条件、停顿、吞吐）。
...
- **逃逸分析**：通过逃逸分析技术，执行引擎可以确定对象是否逃逸出方法的作用域，从而进行一些针对性的优化，如栈上分配、标量替换、同步消除等。
...
如果**确定一个对象不会逃逸出方法之外，那让这个对象在栈上分配内存将会是一个很不错的主意，对象所占用的内存空间就可以随栈帧出栈而销毁**。在一般应用中，不会逃逸的局部对象所占的比例很大，如果能使用栈上分配，那大量的对象就会随着方法的结束而自动销毁了，垃圾收集系统的压力将会小很多。
...
**如果逃逸分析能够确定一个变量不会逃逸出线程**，无法被其他线程访问，那么这个变量的读写肯定就不会竞争，**对这个变量实施的同步措施就可以安全的消除**。

### 标量替换

如果一个变量是基本数据类型，不可拆分，他就被称为标量。**把一个java对象拆散，将其用到的成员变量恢复为原始类型来访问，这个过程就称为标量替换**。假如逃逸分析能够证明一个变量不会被方法外部访问，并且这个对象可以被拆散，那么可以不创建对象，直接用创建若干个成员变量代替，可以让对象的成员变量在栈上分配和读写。
...
* **JVM配置不合理**：内存分配过小，导致内存溢出。
#### 如何处理
1. **增加可用内存**：如果内存溢出是因为系统配置不足导致的，可以考虑增加系统内存或调整JVM的内存配置参数。
2. **优化代码和内存使用**：检查应用程序的代码，确保内存使用效率高，避免不必要的对象创建和持有，及时释放不再需要的对象。可以通过使用内存分析工具来帮助找出内存使用问题。
3. **调整JVM参数**：调整Java虚拟机（JVM）的参数，例如堆大小、永久代（如果使用旧版本的JVM）、垃圾回收器类型和参数等，以优化内存使用和垃圾回收效率。
4. **处理内存泄漏**：如果内存溢出是由内存泄漏引起的，需要对代码进行深入的分析和调试，找出内存泄漏的原因，并进行相应的修复。
### 生产环境应该给服务器分配多少内存合适？
1. **应用程序的性能需求**：不同的应用程序对内存的需求不同。某些应用程序可能需要大量内存来处理大规模的数据，而其他应用程序可能对内存的需求较低。
2. **并发用户数量**：服务器需要足够的内存来处理并发用户的请求。如果服务器同时需要处理大量用户的请求，就需要分配更多的内存。
3. **数据量和数据处理**：处理大量数据的应用程序需要更多的内存来存储和处理数据。如果应用程序需要进行大规模的数据分析、处理或者缓存，就需要更多的内存。
4. **应用程序的架构和技术栈**：不同的应用程序架构和技术栈对内存的需求也不同。例如，使用Java的应用程序可能需要更多的内存来运行JVM，而使用轻量级的语言和框架可能需要较少的内存。
5. **预算和成本考虑**：分配更多的内存通常会增加服务器的成本。因此，需要在性能需求和成本之间进行权衡，以找到适合的内存配置。

我们公司一般分配内存的60% - 75%作为堆内存。
### 如何对垃圾回收器的性能进行调优？
[怎么选择垃圾回收器？](#怎么选择垃圾回收器？)
### 生产环境应该给应用分配多少线程合适？
...
User time是指程序在用户空间执行的CPU时间，也称为CPU执行时间。  **它仅计算程序在用户代码中执行所花费的时间**，不包括系统调用和其他内核级操作。  User time反映了应用程序的计算密集型工作量。  
	
### Sys time（系统时间）：  

Sys time是指程序在内核空间执行的CPU时间。  **它计算了程序执行过程中在内核中花费的时间**，比如系统调用、信号处理等操作。  Sys time反映了程序与操作系统之间的交互和系统级操作的开销。  
	
### 总结  

- 1. 单核环境，real 比user + sys大一点，但不会大很多；  
- 2. 多核环境，user + sys 会比real大，如果各个线程任务分配均匀，差不多是GC线程的倍数；  
- 3. `CMS FULL GC` 退化成`serial old`单线程收集就会出现 real 和 user + sys 差不多的情况 ;
### 如果Real Time远大于User Time和Sys Time，有哪些可能？  

当Java应用程序的垃圾回收（GC）线程的耗时（Real Time）远大于用户时间（User Time）和系统时间（Sys Time）时，可能存在以下场景和情况：  

- **大型堆内存**：如果应用程序的堆内存较大，垃圾回收线程需要更多的时间来扫描、标记和清理垃圾对象。这可能导致垃圾回收的耗时增加，进而使得Real Time远大于User Time和Sys Time。  

- **并发标记阶段耗时**：某些垃圾回收器采用并发标记（Concurrent Marking）的方式进行垃圾对象标记。在此阶段，垃圾回收线程与应用程序线程并发执行，但仍然需要一定的时间来完成标记。如果并发标记阶段的耗时很长，那么Real Time会相应增加。  

- **频繁的GC活动**：如果应用程序频繁发生垃圾回收活动，即GC频率高，那么垃圾回收线程的耗时可能会增加。频繁的GC活动可能表明应用程序的内存使用方式不够优化，可能存在内存泄漏、对象生命周期不合理或者堆内存设置不当等问题。  

- **并发收集阶段耗时**：某些垃圾回收器在执行垃圾收集阶段时，需要与应用程序线程并发执行。如果并发收集阶段的耗时较长，那么Real Time也会相应增加。这可能是由于并发收集过程中需要扫描和处理大量的对象或者复杂的内存结构。  

- **长时间的STW**：垃圾回收过程中，通常会发生停顿（Stop-The-World，STW），即暂停应用程序的执行。如果在垃圾回收期间，STW的时间较长，那么Real Time会增加。这可能是因为垃圾回收器需要执行更多的操作，如根扫描、对象标记、整理内存等。  

针对这些情况，可以考虑以下优化措施：  

- 分析和优化内存使用：检查应用程序的内存使用情况，尽量减少内存泄漏和不必要的对象分配，合理管理对象的生命周期。  

- 调整垃圾回收器参数：根据应用程序的特点和需求，选择合适的垃圾回收器，并进行适当的参数调优，以减少GC线程的耗时。  

- 并发垃圾回收器的使用：某些垃圾回收器支持并发执行，可以减少STW的时间，从而减少Real Time。  

- 堆内存调整：根据应用程序的需求和负载特性，调整堆内存的大小，以减少GC活动的频率和耗时。  

- 需要根据具体的应用程序和环境进行分析和优化，以达到更好的性能和资源利用效率。
...
1、 观察垃圾回收情况
* `jstat -gc PID 1000`查看GC次数，时间等信息
* `jmap -histo PID | head -20` 查看堆内存占用空间最大的前20个对象类型，初步查看哪些对象占用了内存

如果每次GC频繁，而且每次回收的内存空间也正常，那说明对象创建速度快导致内存一直占用很高；如果每次回收的内存非常少，那么很可能是内存泄漏导致内存一直无法回收。
...
- 思路  
	- 通过GC日志找出STW的耗时原因。  
	- 分析项目内导致STW耗时的行为。  
	- 对有问题的服务进行抽样并修改相应行为，持续观察和对比。  
	- 得出结论，提出合理的优化建议。  
- 过程  
	- 通过观察GC日志发现STW耗时在清理weak reference上，进一步发现是清理Final Reference对象时耗时较长。  
	- 调查FinalReference对象的产生原因，发现是由于Java类实现了finalize()方法，将对象放入Finalizer类中的队列中。  
	- 通过分析工具，比如MAT，进一步分析发现，大部分FinalReference对象都是与数据库和redis连接相关的对象，且进入老年代。  
	- 对redis连接池配置进行分析，发现不合理的配置导致了浮动连接的产生。  
	- 提出了优化配置建议，包括根据项目需求设置连接数量和调整连接池参数。  
		- 一般连接池配置最大连接10就够用了  
- 根因  
	- 连接池配置不合理导致连接长期存活，进入老年代，进而回收时STW过长  
## 如何排查线上metaspace溢出问题？  

metaspace中主要存在类的元信息，可能有一下原因导致metaspace溢出：
* 代理类生成过多
	* 可能是错误的使用了第三方软件，导致生成代理类太多
* 自定义类加载器加载了过多得类
* 内存不足
	* 如果确实需要生成足够的类，那么就考虑增加配置
	* 这种情况一般服务从启动的时候Metaspace的大小就比较稳定

> 线上曾出现过错误使用Aviator表达式工具导致Meataspace OOM 的问题
### 关于 GC日志中 Metadata GC Threshold的错误原因

Metadata GC Threshold表示在给Metaspace分配空间的时候失败了，需要执行Full GC来对Metaspace空间的数据进行清理回收，主要是做类卸载，当类相关的类加载器以及其生成的对象全部卸载的时候类才能被卸载。
...
- 可能的原因  
	...
	- 代码问题
### 内存不足
* 线上机器降配，导致内存减小，老年代存活对象太多，频繁GC
	* 由于代码原因导致老年代一直很高，比如服务启动时加载缓存等等，在进行服务降配的时候，就可能因为CMS的阈值触发，频繁GC
### 内存泄漏

* ThreadLocal未清理导致内存泄露，频繁CMS 
* 连接池配置不合理导致浮动链接过多，并且连接存活时间过长，进入老年代

### 大对象

* 创建大对象的时候，会直接进入老年代，如果空间不足就会导致CMS GC
* 大对象导致的CMS GC 往往伴随着 `Promotion Faild`(对象被晋升的目标区域没有足够的空间存放被晋升对象)，空间担保失败，以及老年代内存碎片导致无法存放大的连续对象  .

### 不规范操作

- 线上节点未拉出执行jmap等指令导致GC  
	- 这种方式会在dump前产生一次GC，这样会导致一些Unreachable objects,无法分析他们的引用路径。
> 线上命令的执行，要先将容器拉出
...
1. 由于GC时间过长，可能时STW时间过长，就会影响业务，常见的问题就是接口超时，我们就出现过服务GC达到500ms导致很多接口超时，这种需要定位GC问题。
2. 服务吞吐量降低
3. 服务不稳定，有宕机风险 

...
* Mixed GC
	* 老年代堆占有率达到阈值会触发
	* 回收部分年轻代 + 部分老年代
	* 与Full GC的区别是，他不是整堆回收，而是根据暂停时间回收部分
* Full GC
	* Mixed GC回收后空间依然不足
	* 是对整堆回收的
![](G1回收过程.jpg)
...
### 生产环境发生了内存溢出该如何处理？
#### 内存溢出可能的原因
* **内存泄漏（Memory Leaks）**：最常见的原因之一。内存泄漏是指应用程序分配的内存不再被使用，但没有被释放。这可能是因为对象的引用没有被正确地释放或者持续被引用导致的。
* **大对象和长期持有**：有时候应用程序会创建大对象或者长期持有对象，导致内存消耗过大。如果这些对象没有被及时释放，就会导致内存溢出。比如说一些缓存没有及时清理。
* **递归调用（Recursion）**：过度递归调用可能会导致堆栈溢出（Stack Overflow），从而间接导致内存溢出。
* **大数据量处理**：处理大数据量时，如果不合理地分配和释放内存，就容易导致内存溢出。
* **内存管理不当**：有时候，应用程序可能会分配过多的内存，或者内存管理不当，例如频繁创建和销毁对象，导致内存碎片化和内存溢出。
* **JVM配置不合理**：有时内存溢出可能是由于系统资源限制引起的，例如操作系统限制了应用程序的内存使用。
### 生产环境应该给服务器分配多少内存合适？
### 如何对垃圾回收器的性能进行调优？
### 生产环境CPU负载飙高该如何处理？
### 生产环境应该给应用分配多少线程合适？

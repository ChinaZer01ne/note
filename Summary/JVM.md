...
- Real time是指从程序开始执行到结束所经过的实际时间，也称为墙钟时间（wall clock time）。  
	

- 它包括了程序执行过程中的所有时间，包括CPU执行时间、I/O等待时间和其他等待时间。  
	

- Real time是最终完成任务所需的总时间，它是用户所感知到的时间。  
...
- User time是指程序在用户空间执行的CPU时间，也称为CPU执行时间。  
	

- 它仅计算程序在用户代码中执行所花费的时间，不包括系统调用和其他内核级操作。  
	

- User time反映了应用程序的计算密集型工作量。  
	
- Sys time（系统时间）：  

- Sys time是指程序在内核空间执行的CPU时间。  
	

- 它计算了程序执行过程中在内核中花费的时间，比如系统调用、信号处理等操作。  
	

- Sys time反映了程序与操作系统之间的交互和系统级操作的开销。  
...
- CMS FULL GC 退化成serial old单线程手机就会出现real和user + sys 差不多的情况  

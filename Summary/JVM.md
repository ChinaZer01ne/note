...
### 关于 Metadata GC Threshold

Metadata GC Threshold表示在给Metaspace分配空间的时候失败了，需要执行Full GC来对Metaspace空间的数据进行清理回收，主要是做类卸载，当类相关的类加载器以及其生成的对象全部卸载的时候类才能被卸载。

### Metaspace的GC
1. 分配空间的时候，虚拟机有一个阈值，如果Metaspace的空间大小超过了这个阈值，那么在新的空间分配的时候，虚拟机首先通过收集可以卸载的类加载器达到空间复用的目的，而不是分配新的空间，这个时候会触发GC。
2. Metaspace的总使用空间达到了MaxMetaspaceSize设置的阈值，或者Compressed Class Space被用光了，如果这次FC真的通过卸载类获得了很多空间
## 如何排查线上内存过高的问题？  

## P0面试优先级（2026）
- `P0-1` 数据模型：ZNode、Session、Watcher、ACL。
- `P0-2` 一致性机制：ZAB、选主流程、过半提交。
- `P0-3` 应用场景：注册中心、分布式锁、配置管理。
- `P0-4` 可靠性边界：脑裂风险、会话超时、羊群效应。
- `P0-5` 运维要点：节点规划、监控指标、故障处理。

## P0常见误区修正（本页已修订）
- Watcher 不是持续监听，默认一次触发需重新注册。
- 临时节点生命周期绑定 Session，不是绑定 TCP 连接本身。
- ZooKeeper 更适合协调元数据，不适合大数据量存储。
...
具体⼯作流程为：客户端在向Zookeeper服务器**注册**的同时，会将Watcher对象存储在客户端的WatcherManager当中。当Zookeeper服务器触发Watcher事件后，会向客户端发送通知，客户端线程从WatcherManager中取出对应的Watcher对象来执⾏**回调**逻辑。
...
> * 掌握Zookeeper的角色：Leader、 Follower、 Observer
> * 掌握ZNode的类型：四种
> 	* **持久节点**（Persistent）：手动删除才消失，用于存储配置。
> 	* **临时节点**（Ephemeral）：会话结束自动删除，用于服务注册（如在线状态）。
> 	* **顺序节点**（Sequential）：名称自动追加全局递增序号（如分布式锁排队）。
> 	* **容器/TTL节点**（≥3.5.3）：自动清理空节点或过期节点8。
> * 掌握Watcher的原理：观察者模式
# Zookeeper应⽤场景
...
**ZooKeeper 采⽤的是推拉相结合的⽅式**：客户端向服务端注册⾃⼰需要关注的节点，⼀旦该节点的数据发⽣变更，那么**服务端就会向相应的客户端发送Watcher事件通知**，**客户端接收到这个消息通知之后，需要主动到服务端获取最新的数据**。

> ZooKeeper推送变更通知，客户端主动拉取。

应⽤在启动的时候都会主动到ZooKeeper服务端上进⾏⼀次配置信息的获取，同时，在指定节点上注册⼀个Watcher监听，配置信息发⽣变更，服务端都会实时通知到所有订阅的客户端，从⽽达到实时获取最新配置信息的⽬的。
...
命名服务（Name Service），其中较为常⻅的就是⼀些分布式服务框架（如RPC、 RMI）中的服务地址列表，通过使⽤命名服务，客户端应⽤能够根据指定名字来获取资源的实体、服务地址和提供者的信息等。

ZooKeeper 提供的命名服务功能能够帮助应⽤系统通过⼀个资源引⽤的⽅式来实现对资源的定位与使⽤。⼴义上命名服务的资源定位都不是真正意义的实体资源——在分布式环境中，上层应⽤仅仅需要⼀个全局唯⼀的名字，类似于数据库中的唯⼀主键。

### Zookeeper命名服务

我们结合⼀个分布式任务调度系统来看看如何使⽤ZooKeeper来实现这类全局唯⼀ID的⽣成。 通过调⽤ZooKeeper节点创建的API接⼝可以创建⼀个**顺序节点**，并且在API返回值中会返回这个节点的完整名字。利⽤这个特性，我们就可以借助ZooKeeper来⽣成全局唯⼀的ID 了
...
Master选举可以说是ZooKeeper最典型的应⽤场景了，我们就结合“⼀种海量数据处理与共享模型”这个具体例⼦来看看 ZooKeeper在集群Master选举中的应⽤场景。
...
如果仅仅只是想实现Master选举的话，那么其实只需要有⼀个能够保证数据唯⼀性的组件即可，例如关系型数据库的主键模型就是⾮常不错的选择。但是，如果希望能够快速地进⾏集群 Master 动态选举，那么就可以基于 ZooKeeper来实现。
...
```mermaid
flowchart TD
A[Client写请求] --> B(Follower转发Leader)
B --> C[Leader生成Proposal+ZXID]
C --> D[广播Proposal]
D --> E{Follower持久化+ACK}
E -->|过半ACK| F[Leader发送Commit]
F --> G[所有节点应用事务]
```

### 崩溃恢复模式

Leader 故障后，基于 **ZXID（事务ID）+ epoch（任期）** 选举新 Leader，**同步数据**后恢复服务

### 选举机制
- **比较优先级**：epoch > zxid > server id（myid）。
- **过半原则**：集群节点数需为 **2N+1**（如3/5台），故障容忍 **N 台宕机**。
- **脑裂防护**：写操作需过半节点确认，防止网络分区导致数据不一致。
...
# 扩展
## ZK 场景替代策略
- 注册发现：按业务评估迁移到 Nacos 等
- 分布式锁：评估 Redis/ZK 方案权衡
- 配置中心：统一配置治理与发布流程

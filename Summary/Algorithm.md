## P0面试优先级（2026）
- `P0-1` 链表：反转、合并、环检测、K 组翻转、快慢指针。
- `P0-2` 树：前中后序、层序、最近公共祖先、路径和。
- `P0-3` 动态规划：状态定义、转移方程、滚动优化。
- `P0-4` 贪心：区间、跳跃、调度与正确性证明思路。
- `P0-5` 复杂度：时间/空间分析与边界用例设计。

## P0常见误区修正（本页已修订）
- 刷题不等于会面试，必须能口述思路与复杂度。
- DP 不是“套模板”，关键是状态与转移定义。
- 贪心必须证明局部最优可推出全局最优。
